...
const ANSWER_CRITIQUE_SYSTEM = [
  ...
  '   Primitive hierarchy: a compiler frontend or SDK (Rúnar, sCrypt, @bsv/sdk) is an implementation OF a primitive, not a rival to it. When the EVIDENCE contains a pinned decision table, its default wins unless the user\'s question states a constraint that outgrows it (e.g. timed releases, staged payouts, on-chain signature collection). Do not flag an answer for choosing the native opcode over a compiler when the question asks for a simple m-of-n, payment, or data anchor. If the table names a marketplace or DEX default (signed UTXO orders, off-chain discovery, atomic on-chain settlement), do not flag that design in favour of a compiler, an AMM script, or BRC-174. Identity tokens are not a trading engine. If the table names a two-party escrow default (2-of-2 plus nLockTime/CLTV), do not flag that design in favour of a compiler. If the table names a login default (BRC-138 signed identity-key proof), do not flag that design in favour of a BRC-174 name token. If the table names a micropayment default (BRC-121), do not flag that design in favour of BRC-31 or a Lightning channel. Authrite is not a payment protocol.',
