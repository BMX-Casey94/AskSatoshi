...
describe('model chain configuration', () => {
  ...
  it('leads with the paid OpenRouter workhorse, then a paid non-Google backup, then free tiers', () => {
  ...
  it('marks both paid tiers non-free and every overflow tier free', () => {
    const byId = new Map(eligibleTiers(allKeys, false).map((t) => [t.id, t]));
    expect(byId.get('openrouter-flash-lite')!.free).toBeUndefined();
    expect(byId.get('openrouter-deepseek-flash')!.free).toBeUndefined();
  ...
  it('caps output lower on free tiers than on the paid tiers', () => {
    const byId = new Map(eligibleTiers(allKeys, false).map((t) => [t.id, t]));
    expect(byId.get('openrouter-flash-lite')!.maxOutputTokens).toBe(PAID_MAX_OUTPUT_TOKENS);
    expect(byId.get('openrouter-deepseek-flash')!.maxOutputTokens).toBe(PAID_MAX_OUTPUT_TOKENS);
