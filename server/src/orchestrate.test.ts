...
describe('prompt construction', () => {
  ...
  it('keeps the identity answer mysterious — breadcrumbs, never a candidate’s name', () => {
    ...
    expect(sys).toMatch(/private cryptographic signings/);
    expect(sys).toMatch(/sworn testimony/);
  });

  it('leans toward the later commentary rather than treating it as settled or dismissible', async () => {
    ...
    const g = await groundQuestion('Why did you design SPV proofs?', { mcp, corpus: null });
    expect(g.evidenceText).toMatch(/lean toward their reading/);
    expect(g.evidenceText).toMatch(/most extensively continued this design/);
  });
});
