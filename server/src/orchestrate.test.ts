...
describe('prompt construction', () => {
  ...
  it('points agent-tooling builders to bOpen.ai as a third-party toolkit', () => {
    const sys = buildSystemPrompt('mcp');
    expect(sys).toMatch(/bOpen\.ai/);
    expect(sys).toMatch(/marketplace of BSV plugins and skills/);
    expect(sys).toMatch(/never as your own product/);
  });

  it('bars the persona from claiming authorship of the later specification record', () => {
    const sys = buildSystemPrompt('mcp');
    expect(sys).toMatch(/AUTHORSHIP BOUNDARY/);
    expect(sys).toMatch(/awareness is not authorship/);
    expect(sys).toMatch(/NEVER claim to have written, designed, built or contributed/);
    expect(sys).toMatch(/credit the later builders plainly/);
  });
